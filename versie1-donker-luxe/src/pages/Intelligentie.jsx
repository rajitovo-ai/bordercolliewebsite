    ...
    <PageLayout title="Intelligentie en Cognitie" subtitle={intelligentie.intro} icon={Brain}>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InfoCard title={`${intelligentie.chaser.naam} — ${intelligentie.chaser.woorden} woorden`}>
            ...
            <ul className="space-y-2 text-sm list-none">
              {intelligentie.chaser.vaardigheden.map((v, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="text-bc-gold mt-1">◆</span> {v}
                </li>
              ))}
            </ul>
          ...
          <InfoCard title={`${intelligentie.rico.naam} — ${intelligentie.rico.woorden} woorden`}>
            <ul className="space-y-2 text-sm list-none">
              {intelligentie.rico.vaardigheden.map((v, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="text-bc-gold mt-1">◆</span> {v}
                </li>
              ))}
        ...
        <InfoCard title="Algemene Intelligentie (g-factor)">
          ...
          <p className="text-bc-gold font-medium">{intelligentie.gFactor.betekenis}</p>
        ...
        <InfoCard title="Label-Learner Honden">
          ...
          <p className="text-bc-gold font-medium">{intelligentie.labelLearners.belangrijk}</p>
          <p className="text-xs text-bc-gray italic mt-3">Bron: {intelligentie.labelLearners.bron}</p>
        </InfoCard>

        {/* Cognitieve domeinen table */}
        <SectionCard>
          <h3 className="font-serif text-xl font-semibold text-bc-gold mb-4">Cognitieve Domeinen</h3>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-bc-gold/20">
                  <th className="text-left py-3 px-4 text-bc-gold font-semibold">Domein</th>
                  <th className="text-left py-3 px-4 text-bc-gold font-semibold">Prestatie</th>
                  <th className="text-left py-3 px-4 text-bc-gold font-semibold">Bewijskracht</th>
                </tr>
              </thead>
              <tbody>
                {intelligentie.cognitieveDomeinen.map((d, i) => (
                  <tr key={i} className="border-b border-bc-gold/5">
                    ...
                    <td className="py-3 px-4 text-bc-gray">{d.prestatie}</td>
                    <td className="py-3 px-4 text-bc-gray text-sm">{d.bewijs}</td>
                  </tr>
                ))}
              </tbody>
            </table>
